...
class DGCNN_Cls_Encoder(nn.Module):
    def __init__(self, feat_dim=1024, c_dim=128, k=20, classification=False):
        ...
        if self.classification:
            self.linear1 = nn.Linear(feat_dim*2, 512, bias=False)
            self.bn6 = nn.BatchNorm1d(512)
            self.dp1 = nn.Dropout(p=0.5)
            self.linear2 = nn.Linear(512, 256)
            self.bn7 = nn.BatchNorm1d(256)
            self.dp2 = nn.Dropout(p=0.5)
            self.linear3 = nn.Linear(256, 40) # 40 classes in ModelNet40
...
class DGCNN_cls(nn.Module):
    ...
    def __init__(self, c_dim=32, scatter_type='max', 
                 unet=True, unet_kwargs={'depth': 4, 'merge_mode': 'concat', 'start_filts': 32}, unet3d=False, unet3d_kwargs=None, 
                 plane_resolution=64, grid_resolution=64, plane_type=['xz', 'xy', 'yz'], padding=0.1, feat_dim=1024, k=40, classification=False):
        super().__init__()
        self.c_dim = c_dim
        self.classification = classification
        self.dgcnn_encoder = DGCNN_Cls_Encoder(c_dim=self.c_dim, feat_dim=feat_dim, k=k, classification=self.classification)
        ...
        if unet3d:
            # TODO: Uncomment following and add 
...
class LocalDecoder(nn.Module):
    ...
    def __init__(self, c_dim=64,
                 hidden_size=32, n_blocks=5, leaky=False, sample_mode='bilinear', padding=0.1, d_dim=None):
        super().__init__()
        #c_dim = 32
        if d_dim:
            self.c_dim = d_dim
...
class ImplicitAutoEncoder(nn.Module):
    ...
    def __init__(self, encoder='dgcnn', device=None, classification=False):
        super().__init__()
        ...
        if encoder == 'dgcnn':
            encoder = DGCNN_cls(c_dim=c_dim, unet=unet, unet_kwargs=unet_kwargs,
                                plane_type=plane_type, plane_resolution=plane_resolution,
                                padding=padding, k=k, feat_dim=feat_dim, classification=self.classification)

        # Decoder params
        sample_mode='bilinear'
        hidden_size=32
        if not self.classification:
            decoder = LocalDecoder(c_dim=c_dim, sample_mode=sample_mode, 
                                   hidden_size=hidden_size, padding=padding)
